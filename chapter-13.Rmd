...
best_mlp <- select_best(mlp_tune, metric = "roc_auc")
best_mlp

model_predictions <- 
 collect_predictions(mlp_tune, parameters = best_mlp) %>% 
 mutate(model = "neural network") %>% 
 select(-hidden_units, -penalty)

overlay_roc_curves(model_predictions, highlight = "neural network") 
```
...
model_predictions <- 
 ...
 select(all_of(names(model_predictions))) %>% 
 bind_rows(model_predictions)
