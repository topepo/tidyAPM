...
all_pca_rec <- 
 ...
 step_YeoJohnson(all_predictors()) %>% 
 step_normalize(all_predictors()) %>% 
 step_pca(all_predictors(), num_comp = 3, id = "pca") %>% 
 prep()
...
filtered_rec <- 
    recipe(class ~ ., data = cell_train) %>% 
    step_lincomb(all_predictors(), id = "nzv") %>% 
    prep()
...
filtered_corr_rec <- 
    recipe(class ~ ., data = cell_train) %>% 
    step_corr(all_predictors(), threshold = 0.5, id = "cor filter") %>% 
    prep()
...
penguin_rec <- 
  ...
  step_dummy(all_nominal()) %>% # <- captures factor or character cols
  prep()
